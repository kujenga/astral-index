...
7. **LLM judges** — 12 total judges: 9 intrinsic in `astral_eval.scorers.llm_judges` + 3 reference in `astral_eval.scorers.reference_judges`. The 5 standard judges route through `get_llm_proxy().judge()` (Braintrust AI Proxy or direct OpenAI), falling back to Claude Haiku via Anthropic SDK. The 4 thinking-mode judges use Claude Sonnet with extended thinking and 3-call ensemble median aggregation — Anthropic SDK only, no proxy path. The 3 reference judges (oi_topic_overlap, oi_editorial_depth, oi_structural_similarity) compare against Orbital Index issues and return None when no `expected` text is available.
